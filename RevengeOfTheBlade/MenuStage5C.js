/*  MENU SCENE after Stage5 completed

    Has options for entering the tutorial or starting the final boss scene.
*/

var boss5Button, tutorialButton, audioButton;
var soundState = 'off';
var titleback, source, canvas, ctx, imageData;

class MenuStage5C extends Phaser.Scene {
    constructor() {
        super({ key: 'MenuStage5C' });
    }
    ...
    create() {
      ...
      // boss crosses
      // level 1 boss
      this.add.image(518, 370, 'check');
      // level 2 boss
      this.add.image(530, 460, 'check');


      this.add.text(300,330,'Stone Level', { fontSize: '30px', fill: '#7C5EBF' });
      this.add.text(360, 360, 'BOSS 1: Iron', { fontSize: '20px', fill: '#8678A5' });
      this.add.text(280,420,'Titanium Level', { fontSize: '30px', fill: '#7C5EBF' });
      this.add.text(360,450,'BOSS 2: Steel', { fontSize: '20px', fill: '#8678A5' });
      this.add.text(220,508,'Chromium Alloy Level', { fontSize: '30px', fill: '#7C5EBF' });

      boss5Button = this.add.text(355,538,'Final Boss', { fontSize: '20px', fill: '#8db9d9' });
      boss5Button.setInteractive();
      boss5Button.on('pointerdown', () => {
        buttonSound.play();
        this.soundtrack.stop();
        this.scene.stop('MenuStage5C');
      ...
      boss5Button.on('pointerover', () => { this.buttonOver(boss5Button); });
      boss5Button.on('pointerout', () => { this.buttonNotOver(boss5Button); });
    ...
    buttonOver(button) {
      if (button == boss5Button) {
        boss5Button.setStyle({fill:'#37F121'});
      }
    ...
    buttonNotOver(button) {
      if (button == boss5Button) {
        boss5Button.setStyle({fill:'#8db9d9'})
      }
      if (button == tutorialButton) {
